<?php
/**
 * Fox Shop functions and definitions
 *
 **
...
if ( ! function_exists( 'fox_shop_setup' ) ) :

	function fox_shop_setup() {

		load_theme_textdomain( 'fox-shop', get_template_directory() . '/languages' );
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'fox-shop' ),
			'menu-2' => esc_html__( 'Footer', 'fox-shop' ),
		) );
		 ...
		 */
		 add_filter('get_custom_logo', 'fox_shop_change_logo_class');
		 
		 function fox_shop_change_logo_class ($html){
		 ...
		 */
		 function fox_shop_display_logo(){
		 ...
		 /**
		 * Display Fox Shop custom comments format HTML Output.
		 */
		 function fox_shop_comments_format($comment, $args, $depth){
			  ...
			  echo '<li '; comment_class('x'); echo' id="li-comment-'; comment_ID(); echo '">';
				...
				echo '<div class="message">
						...
						echo '</div>
							...
							/* Comment Message */
							$comment_message = __( 'Your comment is awaiting moderation.', 'fox-shop' );
		 ...
		 */
		 function fox_shop_get_the_comments_navigation( $args = array() ) {
			$navigation = '';
		 
			// Are there comments to navigate through?
			if ( get_comment_pages_count() > 1 ) {
				$args = wp_parse_args( $args, array(
					'prev_text'          => __( 'Older comments', 'fox-shop' ),
					'next_text'          => __( 'Newer comments', 'fox-shop' ),
					'screen_reader_text' => __( 'Comments navigation', 'fox-shop' ),
				) );
		...
		function fox_shop_get_the_single_posts_navigation( ) {
			...
			if($next_link = get_next_post()){ // Display next post link if it exists
				echo '<div class="navigation pull-right">';
					$next_btn_title =  __( 'Next', 'fox-shop' );
		 ...
		 */
		function fox_shop_get_the_posts_navigation( $args = array() ) {
			$navigation = '';
		 
			// Don't print empty markup if there's only one page.
			if ( $GLOBALS['wp_query']->max_num_pages > 1 ) {
				$args = wp_parse_args( $args, array(
					'prev_text'          => __( 'Older posts', 'fox-shop' ),
					'next_text'          => __( 'Newer posts', 'fox-shop' ),
					'screen_reader_text' => __( 'Posts navigation', 'fox-shop' ),
				) );
...
add_action( 'after_setup_theme', 'fox_shop_setup' );
 ...
 */
function fox_shop_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'fox_shop_content_width', 640 );
}
add_action( 'after_setup_theme', 'fox_shop_content_width', 0 );
 ...
 */
function fox_shop_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'fox-shop' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'fox-shop' ),
...
add_action( 'widgets_init', 'fox_shop_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function fox_shop_scripts() {
	...
	wp_enqueue_style( 'fox-shop-style', get_stylesheet_uri() );
	
	wp_style_add_data( 'fox-shop-style', 'rtl', 'replace' );

	wp_enqueue_style( 'fox-shop-blue', get_template_directory_uri() . '/style/css/color/blue.css', array(), '20180131', 'all' );

	// Add Fonts
	wp_enqueue_style( 'fox-shop-google-fonts', 'http://fonts.googleapis.com/css?family=Raleway:400,300,500,600,700,800,900' );
	...
	wp_enqueue_script( 'fox-shop-scripts', get_template_directory_uri() . '/style/js/scripts.js', array( 'jquery' ), '20240318', true );

	wp_enqueue_script( 'fox-shop-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'fox-shop-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'fox_shop_scripts' );

/**
 * Enqueue fonts for gutenberg.
 */
function fox_shop_block_editor_fonts(){
	wp_enqueue_style( 'fox-shop-block-editor-fonts', 'http://fonts.googleapis.com/css?family=Raleway:400,300,500,600,700,800,900' );
}
add_action( 'enqueue_block_editor_assets', 'fox_shop_block_editor_fonts' );
...
/**
 * Customize Fox Shop elipsis at the end of excerpts from " [...]" to just "..." .
 */
 function fox_shop_excerpt_more( $more ) {
...
add_filter('excerpt_more', 'fox_shop_excerpt_more');

 /**
 * Customize Fox Shop Search form input and submit button .
 */
 function fox_shop_get_search_form($form) {
 ...
 add_filter('get_search_form', 'fox_shop_get_search_form');
 
/**
 * Fox Shop walker nav class.
 */
class Fox_Shop_Walker_Nav_Primary extends Walker_Nav_menu {
