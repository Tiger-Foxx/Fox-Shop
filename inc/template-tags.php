...
if ( ! function_exists( 'fox_shop_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function fox_shop_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'fox-shop' ) );
			if ( $tags_list ) {
...
if ( ! function_exists( 'fox_shop_post_thumbnail' ) ) :
/**
 ...
 */
function fox_shop_post_thumbnail() {
...
if ( ! function_exists( 'fox_shop_post_category' ) ) :
	/**
	 * Prints comma seperated post categories for pages.
	 */
	function fox_shop_post_category() {
		
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'fox-shop' ) );
			if ( $categories_list ) {
				/* list of categories. */
...
if ( ! function_exists( 'fox_shop_posted_author' ) ) :
	/**
	 * Prints HTML with meta information for the current post author.
	 */
	function fox_shop_posted_author() {
		/* post author. */
...
if ( ! function_exists( 'fox_shop_post_comments_count' ) ) :
	/**
	 * Prints count of post comments.
	 */
	function fox_shop_post_comments_count() {
		...
		if( ! post_password_required() && ( comments_open() || get_comments_number() ) ){
			...
			$post_comments_count = sprintf( 
				...
				esc_html( _nx( '%1$s Comment ', '%1$s Comments ', $post_comments_count_number, 'comment count number', 'fox-shop' ) ),
...
if ( ! function_exists( 'fox_shop_post_edit_link' ) ) :
	/**
	 * Prints post edit link.
	 */
	function fox_shop_post_edit_link() {
		
		edit_post_link(
			sprintf(
				wp_kses(
					/* translators: %s: Name of current post. Only visible to screen readers */
					__( 'Edit Post <span class="screen-reader-text">%s</span>', 'fox-shop' ),
					array(
						'span' => array(
							'class' => array(),
						),
