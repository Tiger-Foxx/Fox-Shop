...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="post-content image-caption">
		...
		<header class="entry-header">
			...
			$fox_shop_sticky_post_icon = ( is_sticky() ) ? '<i class="fa fa-thumb-tack" title="Sticky Post" style="color: #3f8dbf;"></i>' : '' ;
			
			if ( is_singular() ) :
				the_title( '<h1 class="entry-title post-title">', '</h1>' );
			else :
				the_title( '<h2 class="entry-title post-title">' . $fox_shop_sticky_post_icon . ' <a href="' . esc_url( get_permalink() ) . '" rel="bookmark">', '</a></h2>' );
			endif;
				...
				if ( is_singular() ) { // Return full post content for an existing post of any single post type
					...
					echo '<div class="entry-content">';
						...
						wp_kses(
							...
							__( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'fox-shop' ),
								array(
									'span' => array(
										'class' => array(),
									),
						...
						wp_link_pages( array(
							'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'fox-shop' ),
							'after'  => '</div>',
						) );
					echo '</div><!-- .entry-content -->';
					
					echo '<footer class="entry-footer">';
						fox_shop_entry_footer();
